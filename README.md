...
2. **阶梯训练计划** — L0–L6 关卡式递进，每关有目标、任务与达标线；参与者的**进步档案**保存在浏览器本地（localStorage），免登录。
3. **经典案例库** — 从网络摘录的真实选手、赛事、解法与器材案例，分类并标注出处。
...
完整生产部署（Docker 单容器 + nginx + HTTPS）见 **[DEPLOY.md](./DEPLOY.md)**，包含逐步命令、证书申请/续期与排错。
